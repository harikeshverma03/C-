...
ComponentNode *fastest = nullptr;
map<ComponentNode *, int> mp;
void getfast(ComponentNode *rootComponent){
    if(!rootComponent) return;
    int maxi = INT_MAX, sum = 0;
    if(rootComponent->components.size() == 0) return;
    for(int i = 0; i < rootComponent->components.size(); i++){
        sum+=rootComponent->components[i].value;
    }
    maxi = sum/ rootComponent->components.size();
    mp[rootComponent] = maxi;
    if(!fastest) {
        fastest = rootComponent;
    }
    else{
        if(mp[fastest] < maxi)
            fastest = rootComponent;
...
ComponentNode *getFastest(ComponentNode *rootComponent){
    getfast(rootComponent);
    return fastest;
}
