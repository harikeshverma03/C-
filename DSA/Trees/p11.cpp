...
void solve(){
    ...
    int sum = vec[0].second, count = 1, mini = vec[0].first, ans;
    ans = vec[0].first*vec[0].second;
    for(int i = 1; i < n; i++){
        if(vec[i].first*vec[i].second > ans){
            ans = vec[i].first*vec[i].second;
            sum = vec[i].second;
            ans = vec[i].first;
            count = 1;
        }
        else if((sum+vec[i].second)*vec[i].first > ans && count < k){
