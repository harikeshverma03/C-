...
bool goods(int n){
     ll temp = n;
        ...
        reverse(all(st));
        bool got=true;
        for(ll j=0,k=st.size()-1;j<k;j++,k--){
            if(st[j]!=st[k]) got=false;
        }
        return got;
}
//vector<int> ans;
void back(int n, bool &flag, int k, int sum, vector<int> &res){
    if(flag) return;
    if(res.size() > 12){
        return;
    }
    ...
    for(int i = 0; i < k; i++){
        res.push_back(good[i]);
        back(n, flag, k, sum + good[i], res);
        res.pop_back();
    }
}
void solve(){
