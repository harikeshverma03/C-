...
void findTopoSort(int node, int vis[], stack<int> &st, vector<vector<ll>> &adj, vector<ll> &topo) {
    vis[node] = 1; 
    for(auto it : adj[node]) {
        if(!vis[it]) {
            findTopoSort(it, vis, st, adj, topo); 
        }
    }
    topo.push_back(node);
    st.push(node);
...
ll ans = 0;
void dfs(int node, vector<ll> &visited, vector<vector<ll>> &adj, vector<ll> weights){
    visited[node] = 1;
    ans = ans + weights[node-1];
    ll maxi = INT_MIN;
    ll start = 0;
    for(auto it: adj[node]){
        if(weights[it-1] > maxi && visited[it]!=1){
            start = it;
        }
    }
    if(start) dfs(start, visited, adj, weights);
}
void solve(){
    ans = 0;
    ll n; cin >> n;
	...
	for (int i = 1; i <= n; i++) 
		if (!vis[i]){ 
			findTopoSort(i, vis, st, adj, topo);
            if(topo.size() == n)
                break;
            else
                topo.clear();
        }
    int start = topo.front();
    vector<ll> visited(n+1, 0);
    dfs(start, visited, adj, vec);
    cout << ans << endl;
}
